...
const createIfNotExistUser = async function(userInfo) {
  let user;
  [err, user] = await to(User.findOne({ where: { UserID: userInfo.UserID } }));
  if (user) {
    console.log("createIfNotExistUser found one for", userInfo.UserID);
  } else {
    [err, user] = await to(User.create(userInfo));
    console.log("createIfNotExistUser create new one", user);
    if (err) TE(err);
  }
  return user;
};
module.exports.createIfNotExistUser = createIfNotExistUser;
